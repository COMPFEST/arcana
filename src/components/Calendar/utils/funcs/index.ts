...
export const getMonthID: (monthNumber: number) => MonthTypeID = (
  monthNumber
) => {
  switch (monthNumber) {
    case 0:
...
export const getMonthEN: (monthNumber: number) => MonthTypeEN = (
  monthNumber
) => {
  switch (monthNumber) {
    case 0:
...
export const dateToStringID = (date: Date, withHours = false): string => {
  ...
  if (withHours)
    return `${dateInt} ${monthString} ${yearInt} ${hours
      .toString()
...
export const compareDateEquals = (
  firstDate: Date,
  secondDate: Date
): boolean => {
